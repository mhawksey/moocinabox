...
function my_post_limits( $limit ) {
	...
	if (isset($_GET["posts_per_page"])) { $posts  = $_GET["posts_per_page"]; } else { $posts=20; }; 
	$start_from = ($page-1) * $posts; 
	if (isset($_GET['posts_per_page']) || isset($_GET['page'])){	
        return 'LIMIT '.$start_from.', '.$posts;
    }
    return $limit;
}
